...
function displayNameFromAuthor(author) {
  ...
  return author.displayName || author.nickname || author.globalName || author.username || author.tag || 'Unknown';
}
...
function buildMentionMaps(messages, savedMaps = {}) {
  const users = {};
  const roles = {};

  if (savedMaps.users) {
    for (const [id, user] of Object.entries(savedMaps.users)) {
      users[id] = displayNameFromAuthor(user);
    }
  }

  if (savedMaps.roles) {
    for (const [id, role] of Object.entries(savedMaps.roles)) {
  ...
  for (const message of messages) {
    if (message.author?.id) {
      users[message.author.id] = displayNameFromAuthor(message.author);
    }
  }
...
function renderContentWithMentions(content, mentionMaps) {
  if (!content) return null;

  const users = mentionMaps?.users || {};
  const roles = mentionMaps?.roles || {};
  ...
  while ((match = mentionRegex.exec(content)) !== null) {
    ...
    if (type === '@&') {
      const roleName = roles[id] || 'Role';

      parts.push(
        <span className="mention roleMention" key={`role-${id}-${match.index}`}>
          @{roleName} ({id})
        </span>
      );
    } else {
      const name = users[id] || 'User';

      parts.push(
        <span className="mention" key={`user-${id}-${match.index}`}>
          @{name} ({id})
        </span>
      );
    }
...
function renderDiscordMarkdown(content, mentionMaps) {
  if (!content) return null;
  ...
  return String(content).split('\n').map((line, index) => {
    ...
    while ((match = boldRegex.exec(text)) !== null) {
      if (match.index > last) pieces.push(renderContentWithMentions(text.slice(last, match.index), mentionMaps));
      pieces.push(<strong key={`bold-${index}-${match.index}`}>{renderContentWithMentions(match[1], mentionMaps)}</strong>);
      last = boldRegex.lastIndex;
    }

    if (last < text.length) pieces.push(renderContentWithMentions(text.slice(last), mentionMaps));
...
function renderComponent(component, mentionMaps, key) {
    ...
    return (
      ...
      >
        {Array.isArray(component.components) &&
          component.components.map((child, index) => renderComponent(child, mentionMaps, `${key}-container-${index}`))}
      </div>
    );
  }
    ...
    return (
      <div className="cv2Section" key={key}>
        <div className="cv2SectionText">
          {Array.isArray(component.components) &&
            component.components.map((child, index) => renderComponent(child, mentionMaps, `${key}-section-${index}`))}
        </div>
        {component.accessory && <div className="cv2Accessory">{renderComponent(component.accessory, mentionMaps, `${key}-accessory`)}</div>}
      </div>
    );
  }

  if (component.type === 10 && component.content) {
    return <div className="cv2Text" key={key}>{renderDiscordMarkdown(component.content, mentionMaps)}</div>;
  }
  ...
  if (component.type === 1) {
    return (
      <div className="cv2ActionRow" key={key}>
        {Array.isArray(component.components) &&
          component.components.map((child, index) => renderComponent(child, mentionMaps, `${key}-row-${index}`))}
      </div>
    );
  }
  ...
  if (Array.isArray(component.components)) {
    return <div className="cv2Nested" key={key}>{component.components.map((child, index) => renderComponent(child, mentionMaps, `${key}-nested-${index}`))}</div>;
  }

  return null;
}

function renderComponentsV2(components, mentionMaps, messageId) {
  ...
  return (
    <div className="componentsV2">
      {hasContainer
        ? components.map((component, index) => renderComponent(component, mentionMaps, `${messageId}-component-${index}`))
        : <div className="cv2Card">{components.map((component, index) => renderComponent(component, mentionMaps, `${messageId}-component-${index}`))}</div>}
    </div>
  );
}
...
export default function Transcript({ record }) {
  ...
  const mentionMaps = buildMentionMaps(messages, t.mentionMaps || {});
  ...
  return (
    <main className="page">
            ...
            return (
              <article className="message" key={message.id}>
                ...
                <div className="messageBody">
                  ...
                  {message.content && !hasComponents && <div className="content">{renderContentWithMentions(message.content, mentionMaps)}</div>}

                  {hasComponents && renderComponentsV2(message.components, mentionMaps, message.id)}
                    ...
                    <div className="embeds">
                      {message.embeds.map((embed, index) => (
                        <div className="embed" key={message.id + '-embed-' + index}>
                          ...
                          {embed.description && <div className="embedDescription">{renderContentWithMentions(embed.description, mentionMaps)}</div>}
                          {Array.isArray(embed.fields) && embed.fields.map((field, fieldIndex) => (
                            <div className="embedField" key={fieldIndex}><b>{field.name}</b><p>{renderContentWithMentions(field.value, mentionMaps)}</p></div>
                          ))}
                        </div>
                      ))}
                    </div>
      ...
      <style jsx global>{`
        ...
        .roleMention { background: rgba(88,101,242,.22); color: #c9cdfb; }
