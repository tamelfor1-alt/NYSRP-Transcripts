...
function isImageAttachment(attachment) {
  const type = attachment && attachment.contentType ? attachment.contentType : '';
...
function componentTextExists(components) {
  ...
  return components.some(component => {
    ...
    if (component.media && component.media.url) return true;
...
function renderComponentV2(component, userMap, key) {
  ...
  if (component.type === 10 && component.content) {
    return (
      <div className="componentText" key={key}>
        {renderContentWithMentions(component.content, userMap)}
      </div>
    );
  }
  ...
  if (component.type === 17) {
    return (
      <div className="componentContainer" key={key} style={{ borderLeftColor: component.accent_color ? `#${Number(component.accent_color).toString(16).padStart(6, '0')}` : undefined }}>
        {Array.isArray(component.components) && component.components.map((child, index) =>
          renderComponentV2(child, userMap, `${key}-section-${index}`)
        )}
      </div>
    );
  }
  ...
  if (component.type === 1) {
    return (
      <div className="componentRow" key={key}>
        {Array.isArray(component.components) && component.components.map((child, index) =>
  ...
  if (component.type === 2) {
    ...
    if (component.style === 5 && component.url) {
      return (
        <a className="componentButton linkButton" href={component.url} target="_blank" rel="noreferrer" key={key}>
          {label}
        </a>
      );
    }
    ...
    return (
      <span className="componentButton" key={key}>
        {label}
      </span>
    );
  }
  ...
  if (component.type === 3 || component.type === 8) {
    return (
      <div className="componentSelect" key={key}>
        ...
        {Array.isArray(component.default_values) && component.default_values.length > 0 && (
          <div className="selectOptions">
            {component.default_values.map((value, index) => (
              <span className="selectOption" key={`${key}-default-${index}`}>
                {value.id ? `#${value.id}` : String(value)}
              </span>
            ))}
          </div>
        )}
        ...
        {component.items.map((item, index) => {
          ...
          return (
            <a className="componentMedia" href={url} target="_blank" rel="noreferrer" key={`${key}-media-${index}`}>
              {isImageUrl(url) ? <img src={url} alt="media" /> : <span>Media attachment</span>}
            </a>
          );
        })}
      </div>
  ...
  if (component.media && component.media.url) {
    const url = component.media.url;
    return (
      <a className="componentMedia" href={url} target="_blank" rel="noreferrer" key={key}>
        {isImageUrl(url) ? <img src={url} alt="media" /> : <span>Media attachment</span>}
      </a>
    );
  }
