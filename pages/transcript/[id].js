...
function isImageAttachment(attachment) {
  ...
  const name = (attachment && (attachment.name || attachment.filename)) || '';
...
function renderDiscordMarkdown(content, userMap) {
  ...
  return lines.map((line, index) => {
    ...
    while ((match = boldRegex.exec(text)) !== null) {
      if (match.index > last) {
        parts.push(renderContentWithMentions(text.slice(last, match.index), userMap));
      }

      parts.push(<strong key={`bold-${index}-${match.index}`}>{renderContentWithMentions(match[1], userMap)}</strong>);
      last = boldRegex.lastIndex;
    }

    if (last < text.length) {
      parts.push(renderContentWithMentions(text.slice(last), userMap));
    }
