...
function renderDiscordMarkdown(content, userMap) {
  ...
  return lines.map((line, index) => {
    if (line.trim() === '_ _') {
      return <div className="cv2Spacer" key={`space-${index}`} />;
    }
...
function renderComponent(component, userMap, key) {
  ...
  // 17 = Container. This is the Discord card.
      ...
      >
        {Array.isArray(component.components) &&
          component.components.map((child, index) =>
            renderComponent(child, userMap, `${key}-container-${index}`)
          )}
      </div>
    );
  }
    ...
    return (
      <div className="cv2Section" key={key}>
        <div className="cv2SectionText">
          {Array.isArray(component.components) &&
            component.components.map((child, index) =>
              renderComponent(child, userMap, `${key}-section-${index}`)
            )}
        </div>

        {component.accessory && (
          <div className="cv2Accessory">
            {renderComponent(component.accessory, userMap, `${key}-accessory`)}
          </div>
        )}
      </div>
    );
  ...
  if (component.type === 11 && component.media?.url) {
    return (
      <a className="cv2Thumbnail" href={component.media.url} target="_blank" rel="noreferrer" key={key}>
        <img src={component.media.url} alt={component.description || 'thumbnail'} />
      </a>
    );
  }
    ...
    return (
      <div className="cv2MediaGrid" key={key}>
        {component.items.map((item, index) => {
          ...
          return (
            <a className="cv2Media" href={url} target="_blank" rel="noreferrer" key={`${key}-media-${index}`}>
              <img src={url} alt={item.description || 'media'} />
            </a>
          );
        })}
      </div>
  ...
  if (component.type === 13 && component.file?.url) {
    return (
      <a className="cv2File" href={component.file.url} target="_blank" rel="noreferrer" key={key}>
  ...
  if (component.type === 14) {
    ...
    return visible
      ? <div className={`cv2Separator ${spacing}`} key={key} />
      : <div className={`cv2Spacer ${spacing}`} key={key} />;
  }
  ...
  if (component.type === 1) {
    return (
      <div className="cv2ActionRow" key={key}>
        {Array.isArray(component.components) &&
          component.components.map((child, index) =>
            renderComponent(child, userMap, `${key}-row-${index}`)
          )}
      </div>
    );
  }
  ...
  if (component.type === 2) {
    ...
    if (component.style === 5 && component.url) {
      return (
        <a className={`cv2Button ${styleClass}`} href={component.url} target="_blank" rel="noreferrer" key={key}>
          {label}
        </a>
      );
    }
    ...
    return (
      <span className={`cv2Button ${styleClass}`} key={key}>
        {label}
      </span>
    );
  }
        ...
        {Array.isArray(component.options) && component.options.length > 0 && (
          <div className="cv2SelectOptions">
            {component.options.map((option, index) => (
              <span className="cv2SelectOption" key={`${key}-option-${index}`}>
  ...
  if (Array.isArray(component.components)) {
    return (
      <div className="cv2Nested" key={key}>
        {component.components.map((child, index) =>
          renderComponent(child, userMap, `${key}-nested-${index}`)
        )}
      </div>
    );
  }
    ...
    <div className="componentsV2">
      {hasContainer ? (
        components.map((component, index) =>
          renderComponent(component, userMap, `${messageId}-component-${index}`)
        )
      ) : (
        <div className="cv2Card">
          {components.map((component, index) =>
            renderComponent(component, userMap, `${messageId}-component-${index}`)
          )}
        </div>
